...
<xsl:stylesheet
  ...
  <xsl:template match="/">
    ...
    <Project
      ...
      <PropertyGroup>
        <AssignTargetPathsDependsOn>$(AssignTargetPathsDependsOn);ProtobuildAddNativeBinaries</AssignTargetPathsDependsOn>
      </PropertyGroup>
      <Target Name="ProtobuildAddNativeBinaries" BeforeTargets="AssignTargetPaths">
        <ItemGroup>
          <xsl:for-each select="$project/References/Reference">
            <xsl:variable name="include-name" select="./@Include" />
            <xsl:if test="
